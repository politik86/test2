...
if(isset($configs->geoparams['allowgeo']) && ($configs->geoparams['allowgeo'] == '1')) {
		if(isset($current)) {
			$counter = 1;
			$output = "ADAG(document).ready(function(){ ADAG('.location-info').click(function(event1){ window.setTimeout(function(){";
			if(isset($current->sets)){
				foreach($current->sets as $element) {
					...
					if(($element->type != 'region')&&($element->type != 'city')) {
						...
						if(isset($element->data)) { $temp_set = "'".implode('|', $element->data)."'"; }

						if($element->type == 'region') {
							$output .= " window.setTimeout(function(){ var aux = ".$temp_set."; ";
							...
							for($i=1;$i<=count($element->data)-1;$i++){
								$ttemp2[] = $element->data[$i];
							}
							//echo "<pre>";var_dump($ttemp2);die();
					...
					if($element->type == 'city') {//||($element->type == 'usarea') ||($element->type == 'postalcode')
						//$output .= " window.setTimeout(function(){ ADAG('.maininput').hide(); },1); ";
					}
					$counter++;
				}
			}
			$output .= "},1); ADAG(this).unbind(event1); }); }); ";
    ...
    if(isset($configs->geoparams['allowgeoexisting']) && ($configs->geoparams['allowgeoexisting'] == '1')
        && ((!isset($current->sets))||(count($current->sets) == 0))
        && (isset($_row->channel_id))&&(intval($_row->channel_id)>0) )
    {
