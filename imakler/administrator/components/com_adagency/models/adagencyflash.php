...
class adagencyAdminModeladagencyFlash extends JModelLegacy {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function getExistsZone(){
		$db = JFactory::getDBO();
		$sql = "SELECT zoneid FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%flash%' LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
	...
	function getExistsPackage(){
		...
		$sql = "SELECT tid FROM `#__ad_agency_order_type` AS p WHERE p.location IN (
					SELECT `zoneid` FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%flash%'
				) LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
		if(intval($res) > 0) { return true; } else { return false; }
	...
	function getExistsFlashCampaign(){
		$db = JFactory::getDBO();
	...
	function getExistsZoneWH($w,$h){
		$db = JFactory::getDBO();
		$sql = "SELECT * FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%flash%'";
		$db->setQuery($sql);
