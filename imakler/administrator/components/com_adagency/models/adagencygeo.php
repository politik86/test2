...
class adagencyAdminModeladagencygeo extends JModelLegacy {
	...
	function getPagination(){
		...
		if (empty($this->_pagination))	{
			...
			if (!isset($this->_total)) $this->getChannels();
	...
	function getUsers(){
		$db = JFactory::getDBO();
		$sql = 'SELECT DISTINCT(c.created_by) AS id, u.name, u.username 
				FROM `#__ad_agency_channels` as c 
	...
	function getUL(){
		...
		if(!isset($created_by)) { 
			if(isset($_SESSION['created_by'])) { $created_by = $_SESSION['created_by']; }
		}
		if($users != NULL) {
			$output = '&nbsp;&nbsp;&nbsp;&nbsp;'.JText::_('ADAG_CREATED_BY').'&nbsp;&nbsp;<select name="created_by" onchange="document.topform1.submit();"><option value="YN">'.JText::_('VIEWAD_SEARCH_SELECT').'</option>';
			foreach($users as $element){
				$selected = NULL;
				if($element->id == $created_by) {
					$selected = " selected='selected' ";
				}
				$output .= '<option'.$selected.' value="'.$element->id.'">'.$element->name.' ('.$element->username.')</option>';
			}
			$output .= '</select>';
			return $output;
		} else { return NULL; }
	...
	function getChannels() {
		...
		$db = JFactory::getDBO();
		if(isset($data['search_channel'])) {
			$where = " WHERE c.name LIKE '%".$data['search_channel']."%' ";
			$_SESSION['search_channel'] = $data['search_channel'];
		} elseif(isset($_SESSION['search_channel'])) {
			$where = " WHERE c.name LIKE '%".$_SESSION['search_channel']."%' ";
		} else {
			$where = " WHERE 1=1 ";
		}	
		$public_filter = NULL; $created_filter = NULL;
		$limitstart=$this->getState('limitstart');
		$limit=$this->getState('limit');
		if($limit!=0){
			$limit_cond=" LIMIT ".$limitstart.",".$limit." ";
			if(($limitstart != 0)&&($limitstart < $limit)) { $limit_cond = NULL;}
		} else {
			$limit_cond = NULL;
		}
		
		/* filter by public status - start */
		...
		if(isset($search_public) && $search_public!='YN' && $search_public != NULL){
			$public_filter.=" AND (c.public='".$search_public."')";
		}
		/* filter by public status - stop */

		/* filter by created_by - start */
		$created_by = JRequest::getVar('created_by');
		if(isset($created_by)){
			$_SESSION['created_by']= $created_by;
			$cfilter = $created_by;
		}
		elseif(isset($_SESSION['created_by'])) {
			$cfilter = $_SESSION['created_by'];
		}
		if(isset($cfilter) && $cfilter != 'YN' && $cfilter != NULL){
			$created_filter.=" AND (c.created_by='".$cfilter."')";
		}
		/* filter by created_by - stop */		
		
		$sql = "SELECT c.*, u.name AS uname, u.username FROM `#__ad_agency_channels` AS c LEFT JOIN `#__users` AS u ON c.created_by = u.id ".$where.$public_filter.$created_filter;
		//echo "<pre>";var_dump($sql);echo "</pre><hr />";
		$this->_total = $this->_getListCount($sql);
		$sql .= " ORDER BY ordering ASC ".$limit_cond;
		$db->setQuery($sql);
	...
	function getChannel() {
		...
		if(isset($data['cid'][0])&&($data['cid'][0] != 0)) { $current = intval($data['cid'][0]); }
		elseif(isset($get['cid'][0])&&($get['cid'][0] != 0)) { $current = intval($get['cid'][0]); }
				
		if(isset($current)) {
			$sql = "SELECT * FROM `#__ad_agency_channels` WHERE id =".intval($current);
			$db->setQuery($sql); 
			$result = $db->loadObject();
			
			$sql = "SELECT s.type, s.option, s.logical, s.data FROM `#__ad_agency_channels` AS c, `#__ad_agency_channel_set` AS s WHERE c.id = ".intval($current)." AND c.id = s.channel_id ORDER BY s.id ASC";
			$db->setQuery($sql);
	...
	function saveSettings(){
		...
		$sql = "UPDATE `#__ad_agency_settings` SET `cityloc` = '".trim($data['cityloc'])."' WHERE `id` > 0;";
		$db->setQuery($sql);
		if(!$db->query()){
			return false;
		}
		$sql = "UPDATE `#__ad_agency_settings` SET `countryloc` = '".trim($data['countryloc'])."' WHERE `id` > 0;";
		$db->setQuery($sql);
		if(!$db->query()){
			return false;
		}
		$sql = "UPDATE `#__ad_agency_settings` SET `codeloc` = '".trim($data['codeloc'])."' WHERE `id` > 0;";
		$db->setQuery($sql);
		if(!$db->query()){
			return false;
		}
		unset($data['codeloc']);
		unset($data['countryloc']);				
		unset($data['cityloc']);
		unset($data['task']);
		unset($data['controller']);
		unset($data['boxchecked']);
		unset($data['option']);
		$data = @serialize($data);
		$sql = "UPDATE `#__ad_agency_settings` SET `geoparams` = '".$data."' WHERE `id` > 0;";
		$db->setQuery($sql);
		if(!$db->query()){
			return false;
		}	
	...
	function getSettings(){
		$db = JFactory::getDBO();
		$sql = "SELECT geoparams FROM #__ad_agency_settings WHERE id>0 LIMIT 1";
		$db->setQuery($sql);
	...
	function publish(){
		$db = JFactory::getDBO();
		$data = JRequest::get('post');
		$sql = "UPDATE `#__ad_agency_channels` SET `public` = 'Y' WHERE `id` = '".intval($data['chan'])."' LIMIT 1";
	...
	function unpublish(){
		$db = JFactory::getDBO();
		$data = JRequest::get('post');
		$sql = "UPDATE `#__ad_agency_channels` SET `public` = 'N' WHERE `id` = '".intval($data['chan'])."' LIMIT 1";
		//echo "<pre>";var_dump($sql);die();
		$db->setQuery($sql);
		if(!$db->query()){
			return false;
	...
	function saveChannel(){
		...
		$when = 'NOW()';
		$created_by = $user->id;
		if(isset($data['cid'][0])&&($data['cid'][0] != 0)) {
			$idToEdit = $data['cid'][0];
			$sql = "SELECT * FROM #__ad_agency_channels WHERE id = '".intval($idToEdit)."'";
			$db->setQuery($sql);
			$chan_info = $db->loadObject();
			//echo "<pre>";var_dump($chan_info);echo "<hr />";//die();
			if(isset($chan_info)&&($chan_info != NULL)){
				$when = "'".$chan_info->created."'";
				if(isset($chan_info->created_by)&&($chan_info->created_by != '')) { $created_by = $chan_info->created_by; }
			}
			$this->delete("ok");
		} else {
			$idToEdit='NULL';
		...
		$sql = "INSERT INTO `#__ad_agency_channels` (`id` ,`name`, `banner_id`, `advertiser_id`, `public`, `created`, `created_by`) VALUES (".intval($idToEdit)." , '".$data['cname']."', '".intval($data['banner_id'])."', '".$data['advertiser_id']."', '".$data['public']."', ".$when.", '".$created_by."');";
		...
		if(!$channel_id) {
			$sql = "SELECT * FROM #__ad_agency_channels WHERE `name` = '".$data['cname']."' ORDER BY id DESC LIMIT 1";
			$sqlz[] = $sql;
			$db->setQuery($sql);
			$channel_id = $db->loadResult();
		}
		...
		if(isset($data['limitation'])&&($data['limitation'] != '')) {
			...
			foreach($data['limitation'] as $element){
				if($element['type'] == "city" || $element['type'] == "region"){
					$request_country = trim($element['data']["0"]);
					$request_country = explode(",", $request_country);
					
					if(isset($request_country) && count($request_country) > 0){
						$new_list = array();
						foreach($request_country as $key=>$key_element){
							if(trim($key_element) != ""){
								$code = $this->getCountryCode(trim($key_element));
								$new_list[] = $code;
							}
						}
						$element['data']["0"] = implode(",", $new_list);
					}
					
					if($element['type'] == "city"){
						$request_city = trim($element['data']["1"]);
						$request_city = explode(",", $request_city);
						if(isset($request_city) && count($request_city) > 0){
							$new_list = array();
							foreach($request_city as $key=>$key_element){
								if(trim($key_element) != ""){
									$new_list[] = trim($key_element);
								}
							}
							$element['data']["1"] = implode(",", $new_list);
						}
					}
					elseif($element['type'] == "region"){
						$request_region = trim($element['data']["1"]);
						$request_region = explode(",", $request_region);
						
						if(isset($request_region) && count($request_region) > 0){
							$new_list = array();
							foreach($request_region as $key=>$key_element){
								if(trim($key_element) != ""){
									$new_list[] = $this->getRegionCode(trim($key_element));
								}
							}
							$element['data']["1"] = implode(",", $new_list);
						}
				...
				$temp[] = "(NULL, '".$channel_id."', '".$element['type']."','".$element['logical']."','".$element['option']."', '".json_encode($element['data'])."' )";
			}
	...
	function checkDuplicate(){
		...
		if(isset($data['limitation'])&&($data['limitation'] != '')) {
			$limitation_sql = "INSERT INTO `#__ad_agency_channel_set` (`id` ,`channel_id` ,`type` ,`logical` ,`option` ,`data`) VALUES ";
			$sql = "SELECT s.channel_id FROM #__ad_agency_channel_set AS s, #__ad_agency_channels AS c WHERE s.channel_id = c.id AND c.banner_id =0 ";
			if(isset($cid[0])&&(intval($cid[0])!='0')) {
				$sql.="AND s.channel_id<>".$cid[0]." "; 
				// check for duplicates except for the current channel
			}
			$temp = NULL;
			foreach($data['limitation'] as $element) {
				$extra = "AND (s.type='".$element['type']."' AND s.logical='".$element['logical']."' AND s.option='".$element['option']."' AND s.data='".json_encode($element['data'])."')";
				$sql2 = $sql.$extra;
				$aux[] = $sql2;
				$db->setQuery($sql2);
				$temp[] = $db->loadColumn();				
		...
		if(!empty($temp)){
			...
			if(count($temp)==1) { 
				...
				for($i=0;$i<=count($temp)-2;$i++){
					$final = array_intersect($temp[$i],$temp[$i+1]);
				}
			}
		}
		...
		if(isset($final)&&$final!=NULL){
			foreach($final as $element){
				$sql = "SELECT COUNT(id) FROM #__ad_agency_channel_set WHERE channel_id=".intval($element);
				$db->setQuery($sql);
				$lim_nr = $db->loadResult();
				echo "<b>".$lim_nr."</b> | ";
				if($lim_nr == count($data['limitation'])) { return false; }
			}
		}
		return true;
	}
	
	function getUIDByAid($id) {
		$db =  JFactory::getDBO();
		$sql = "SELECT user_id FROM #__ad_agency_advertis WHERE aid = '".intval($id)."' LIMIT 1 ";
		$db->setQuery($sql);
		$result = intval($db->loadResult());
		
		$sql = "SELECT username FROM #__users WHERE id = ".intval($result)."";
		$db->setQuery($sql);
	...
	function getBannerById($id){
		$db =  JFactory::getDBO();
		$sql = "SELECT * FROM #__ad_agency_banners WHERE id = '".intval($id)."' LIMIT 1 ";
		$db->setQuery($sql);
		$result = $db->loadObject();
		return $result;
	}
	...
	function delete($ok = "") {
		...
		if(isset($data['cid'])) {
			foreach($data['cid'] as $element) {
				$count = 0;
				if($ok == ""){
					$sql = "select count(*) from #__ad_agency_banners where `channel_id`=".intval($element);
					$db->setQuery($sql);
					$db->query();
					$count = $db->loadColumn();
					$count = $count["0"];
				...
				if(intval($count) == 0){
					...
					$app->redirect("index.php?option=com_adagency&controller=adagencyGeo&task=channels", JText::_("ADAG_CHANEL_ASSIGNED_TO_AD"), 'error');
				}
			}
			$in = '('.implode(',',$temp).')';
			$sql1 = 'DELETE FROM #__ad_agency_channels WHERE id IN '.$in;
			$sql2 = 'DELETE FROM #__ad_agency_channel_set WHERE channel_id IN '.$in;
			$db->setQuery($sql1);
	...
	function saveorder($idArray = null, $lft_array = null){
		...
		$table = $this->getTable("adagencygeo");
