	...
	$lang->load($extension, $base_dir, $language_tag, $reload);
									...
									<div class="controls">
									   ...
									   <!--Numeric Range-->
										...
										if($fields->mapping_fieldtype=="gender")
										{
											...
											echo JHTML::_('select.genericlist', $gender, 'mapdata[]['.$fields->mapping_fieldname.',select]', ' class="ad-fields-inputbox chzn-done" id="mapdata[]['.$fields->mapping_fieldname.',select]" size="1"',   'value', 'text', $flds[$fields->mapping_fieldname.',select']);
										}
										if($fields->mapping_fieldtype=="boolean")
										{
											...
											echo JHTML::_('select.genericlist', $boolean, 'mapdata[]['.$fields->mapping_fieldname.',select]', ' class="ad-fields-inputbox chzn-done" id="mapdata[]['.$fields->mapping_fieldname.',select]" size="1"',   'value', 'text', $flds[$fields->mapping_fieldname.',select']);
										}
										/*
											...
											if($fields->mapping_fieldtype=="singleselect")
											{
												...
												if(isset($flds[$fields->mapping_fieldname.',select']) || isset($this->addata_for_adsumary_edit->$singlevar))
												{
													...
													if($this->edit_ad_adsumary)
													{
														...
														echo JHTML::_('select.genericlist', $options, 'mapdata[]['.$fields->mapping_fieldname.',select]', 'class="ad-fields-inputbox chzn-done" size="1" '.$display_reach,   'value', 'text', $mdata);
													}
													else
													echo JHTML::_('select.genericlist', $options, 'mapdata[]['.$fields->mapping_fieldname.',select]', ' class="ad-fields-inputbox chzn-done"'.$display_reach.' id="mapdata[]['.$fields->mapping_fieldname.',select]" size="1"',   'value', 'text', $flds[$fields->mapping_fieldname.',select']);
													$options= array();
												}
												else
												{
													...
													echo JHTML::_('select.genericlist', $options, 'mapdata[]['.$fields->mapping_fieldname.',select]', 'class="ad-fields-inputbox chzn-done"  id="mapdata[]['.$fields->mapping_fieldname.',select]"'.$display_reach.' size="1"',   'value', 'text', '');
													$options= array();
												}
											}
									...
									//Multiselect
												...
												if(isset($flds[$fields->mapping_fieldname.',select']) || isset($this->addata_for_adsumary_edit->$multivar))
													{
														...
														if($this->edit_ad_adsumary)
														{
															...
															echo JHTML::_('select.genericlist', $options, 'mapdata[]['.$fields->mapping_fieldname.',select]', 'class="ad-fields-inputbox chzn-done" id="mapdata[]['.$fields->mapping_fieldname.',select]" size="'.$size.'" multiple="true"'.$display_reach,   'value', 'text', $multidata);
															$options= array();
													}
													else
													{
														...
														{	$size = '6';}
															...
															$size = '3';
														echo JHTML::_('select.genericlist', $options, 'mapdata[]['.$fields->mapping_fieldname.',select]', 'class="ad-fields-inputbox chzn-done"  size="'.$size.'" id="mapdata[]['.$fields->mapping_fieldname.',select]" multiple="true"'.$display_reach,   'value', 'text', '');
											...
											if($fields->mapping_fieldtype=="daterange")
											{
												...
												else
												{
													if($this->edit_ad_adsumary)
													{
															echo JHTML::_('calendar', '', 'mapdata[]['.$fields->mapping_fieldname.'_low|daterange|0]', 'mapdata['.$key.']['.$fields->mapping_fieldname.'_low]', '%Y-%m-%d', array('class'=>'ad-fields-inputbox','onchange'=>'calculatereach()'));
															echo JText::_('SA_TO');
															echo JHTML::_('calendar', '', 'mapdata[]['.$fields->mapping_fieldname.'_high|daterange|1]', 'mapdata['.$key.']['.$fields->mapping_fieldname.'_high]', '%Y-%m-%d', array('class'=>'ad-fields-inputbox',$display_reach_fun));
													}
													else
													{
															echo JHTML::_('calendar', '', 'mapdata[]['.$fields->mapping_fieldname.'_low|daterange|0]', 'mapdata['.$key.']['.$fields->mapping_fieldname.'_low]', '%Y-%m-%d', array('class'=>'ad-fields-inputbox',$display_reach_fun));
															echo JText::_('SA_TO');
															echo JHTML::_('calendar', '', 'mapdata[]['.$fields->mapping_fieldname.'_high|daterange|1]', 'mapdata['.$key.']['.$fields->mapping_fieldname.'_high]', '%Y-%m-%d', array('class'=>'ad-fields-inputbox',$display_reach_fun));
													}
												}
										 ...
										 //date
												if($fields->mapping_fieldtype=="date")
												{
													...
													if(isset($flds[$fields->mapping_fieldname]) || isset($this->addata_for_adsumary_edit->$datevar))
													{
														if($this->edit_ad_adsumary)
														{
															echo JHTML::_('calendar', $this->addata_for_adsumary_edit->$datevar , 'mapdata[]['.$fields->mapping_fieldname.']', 'mapdata['.$key.']['.$fields->mapping_fieldname.']','%Y-%m-%d', array('class'=>'ad-fields-inputbox',$display_reach_fun));
														}
														else
														{
															echo JHTML::_('calendar', $flds[$fields->mapping_fieldname] , 'mapdata[]['.$fields->mapping_fieldname.']',
															'mapdata['.$key.']['.$fields->mapping_fieldname.']','%Y-%m-%d', array('class'=>'ad-fields-inputbox',$display_reach_fun));
														}
												}
												else
												{
						 ...
						 <?php
							...
							<?php
									...
									$results = $dispatcher->trigger('onFrontendTargetingDisplay',$adid);
									$j=1;
