...
//		$previewdata[0]->image = 'images'.DS .'socialads'.DS.basename(JPATH_SITE.$previewdata[0]->image);
		$previewdata[0]->imagesrc = JUri::Base().'images/socialads/'.basename(JPATH_SITE.$previewdata[0]->image);

		$previewdata[0]->image = '<img width="100" src="'.JUri::Base().'images/socialads/'.basename(JPATH_SITE.$previewdata[0]->image).'" />';
...
function parseObjectToArray($object) {
	...
	/*function adsPlaceOrder()
	{
		...
		print"<pre>asfdasdfadsfasd"; print_r($billdata); die;
		jexit();
	} */
	...
	function activateAd()
	{
		...
		if($socialads_config['approval'] && $adminApproval==0)
		{
			$msg = JText::_('COM_SA_AD_CREATED').' '.JText::_('COM_SA_ADMIN_APPROVAL_NOTICE');
		}
		else
		{
			$msg = JText::_('COM_SA_AD_CREATED');
		}
	...
	function draftAd()
	{
		...
		$msg = JText::_('COM_SA_AD_SAVED_DRAFT');
	...
	{
		$socialadshelper = new socialadshelper();
		$socialadshelper->processFreeOrder();
	}
