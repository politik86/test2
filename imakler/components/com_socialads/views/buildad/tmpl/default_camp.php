...
<div class="techjoomla-bootstrap">
	...
	<div class="form-horizontal">
		...
		<div class="control-group">
					...
					if($this->addata_for_adsumary_edit->camp_id )
					{
						$selected_campaign = $this->addata_for_adsumary_edit->camp_id;
					}
					else
					{
						$input = JFactory::getApplication()->input;
						$selected_campaign = $input->get('campid','','INT');
					}
					?>
				 ...
				 <div class="controls">
					<?php echo JHtml::_('select.genericlist', $campselect, "camp",'onchange="hideNewCampaign()"', "value", "text", $selected_campaign);?>
				...
				<div class = "controls ">
					<div id="camp_daily_budget"  style="display:none">
						<p class="text-info">
							&nbsp;<span id="camp_daily_budget_span"></span></p>
					</div>
				</div>
		</div>
 ...
 ?>
		...
		<div class="control-group">
			...
			?>
				...
				<div class="controls">
							<?php echo JHtml::_('select.genericlist', $singleselect, "pricing_opt", 'onchange=getzone_price()', "value", "text",$this->addata_for_adsumary_edit->ad_payment_type )?>
				</div>
				<div class="controls">
...
<script type="text/javascript">
	var campData=<?php
			$teamp=array();
			$teamp[0]=$this->camp_dd;
			echo json_encode($teamp); ?>;
	...
	{
		var selected_campaign = techjoomla.jQuery("#camp option:selected").val();

		document.getElementById('camp_daily_budget').style.display='none';
		if(selected_campaign)
		{
			for(i=0;i<campData[0].length;i++)
			{
				if(selected_campaign==campData[0][i]['camp_id'])
				{
					techjoomla.jQuery('#camp_daily_budget_span').html("<?php echo JText::_('COM_SA_CAMP_DAILY_BUDGET') ?> "+campData[0][i]['daily_budget']+ "<?php echo ' '.$socialads_config['currency']; ?>");
					document.getElementById('camp_daily_budget').style.display='block';
					break;
				}
