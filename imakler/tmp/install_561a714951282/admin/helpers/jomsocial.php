...
class JomSocialTargeting {
	...
	public static function render($ad_id = 0, $type){
		if($type == "list"){
			if (self::exists()) {
			...
			if (self::exists()) {
				...
				echo '<div class="tab-pane" id="jomsoc-info">';
				$options = self::getSelected();
				
				if (count($options) == 0) {
					echo "<a href='index.php?option=com_adagency&controller=adagencyConfigs&task2=jomsocial'>";
					echo	JText::_('ADAG_JOMSOC_ENBL') . "</a>";
					return false;
				}
	...
	public static function render_front($ad_id = 0) {
		if (self::exists()) {
			if (!self::exists_frontend()) { return false; }
			$options = self::getSelected();
			$groups = self::getOptsParents($options);
			...
			echo "<fieldset class='uk-margin-large-top ada-joms-target'>";

			echo "<legend>".JText::_('ADAG_JOMSOC_TARGETING').' '.JText::_('ADG_OPTIONAL')."</legend>";
	...
	public static function render_group_info($group) {
		echo "<h3 class='uk-h3 uk-margin-top-remove'>" . $group->name . "</h3>";
	}
	...
	public static function render_select($opt, $attr = NULL, $multiple = NULL) {
		...
		$output = "<div class='jom_select'>";
		$output .= "<select class='uk-display-inline-block' name='jomsocial[" . $opt->id . "]" . $multiple . "' " . $attr . ">";
		foreach ($_opts as $op) {
	...
	public static function render_radio($opt) {
		$_opts = self::extract_opts($opt->options);
		$output = "<div class='jom_radio'>";
		foreach ($_opts as $op) {
			$output .= "<input type='radio' class='uk-margin-right' name='jomsocial[" . $opt->id . "]' value='" . $op . "' />";
			$output .= "<label>" .JText::_($op). "</label>";
		}
		$output .= "</div>";
		echo $output;
	}
	...
	public static function render_checkbox($ad_id, $opt) {
		...
		foreach ($_opts as $op) {
			...
			$output .= "<input type='checkbox' {$checked} class='uk-margin-right' name='jomsocial[" . $opt->id . "][]' value='" . $op . "' /><span class='lbl'></span>";
	...
	public static function render_age($ad_id, $opt) {
		...
		$output  = "<select class='uk-display-inline-block uk-form-width-mini' name='jomsocial[" . $opt->id . "][]'>";
		$output .= self::getAges(false, $vals[0]);
		$output .= "</select> - <select class='uk-display-inline-block uk-form-width-mini' name='jomsocial[" . $opt->id . "][]'>";
	...
	public static function render_birthdate($opt) {
		...
		$output = "<div class='jom_birthdate'>";
