...
class JomSocialTargetingJSAd {
	...
	public static function render_group_info($group) {
		echo "<div class='jom_groups uk-margin-top-remove uk-margin-bottom'>" . $group->name . "</div>";
	}
	...
	public static function render_radio($opt) {
		...
		foreach ($_opts as $op) {
			...
			$output .= "<input type='hidden' value='".$opt->id."_".$i."' name='hidden_ids' /><br />";
			$i++;
		}
		$output .= "</div>";
		echo $output;
	...
	public static function render_checkbox($ad_id, $opt) {
		...
		foreach ($_opts as $op) {
			...
			$output .= "<input type='checkbox' {$checked} id='jomsocial_".$opt->id."_".$i."' name='jomsocial[" . $opt->id . "][]' value='" . $op . "' onclick='javascript:changeTarget();' /><span class='lbl uk-margin-right' style='margin-bottom:5px;'></span>";
	...
	public static function render_age($ad_id, $opt) {
		...
		$output  = "<div class='uk-form-controls'>";
		$output .= "<select class='uk-display-inline-block' id='jomsocial_".$opt->id."_1' name='jomsocial[" . $opt->id . "][]' onchange='javascript:changeTarget();'>";
		$output .= self::getAges(false, $vals[0]);
		$output .= "</select> - <select class='agency_selectbox' id='jomsocial_".$opt->id."_2' name='jomsocial[" . $opt->id . "][]' onchange='javascript:changeTarget();'>";
	...
	public static function render_birthdate($opt) {
		...
		$output .= "<select class='uk-display-inline-block uk-form-width-mini' id='jomsocial_".$opt->id."' name='jomsocial[" . $opt->id . "]' onchange='javascript:changeTarget();'>";
		$output .= $months;
		$output .= "</select>&nbsp;";
