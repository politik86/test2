...
function initAll(){
    ...
    for (var i=0;i<obj.length;i++) {
        ...
        if(obj[i].className=='boxrow') {
            ...
            obj[i].onmouseout=boxrow_out;

            obj[i].onclick=boxrow_clicked;

            obj[i].firstChild.onclick = function() {
...
window.onload = function() { if(!existsLim()) { document.getElementById('container').style.display = 'none';} }



function boxrow_over(){
...
function boxrow_clicked(){

    if(this.firstChild.nodeName){

        if(this.firstChild.nodeName == "INPUT") {

            if (this.firstChild.checked == false ) {
...
function selim(val, aux){
	...
	if (val == 'continent') {
        ...
        var mycelltwo = document.createElement('TD');
        ...
        mycell.style.padding = "15px 0px";

        mycelltwo.width = "150";

        if(currentid==1) { var display='none';} else { var display='';}

        mycell.innerHTML = '<select name="limitation['+currentid+'][logical]" id="'+myrow.id+'logical" style="display:'+display+';"><option value="OR"><?php echo JText::_('ADAG_OR'); ?></option><option value="AND"><?php echo JText::_('ADAG_AND'); ?></option></select>';

        mycelltwo.align = "center";

        mycelltwo.style.padding = "15px 0px";

        mycelltwo.style.verticalAlign = "top";

        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_CONTINENT'); ?>';

        mycellthree.innerHTML = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="is"><?php echo JText::_('ADAG_IS_ANY'); ?></option><option value="isnot"><?php echo JText::_('ADAG_NOT_IS_ANY'); ?></option></select><p />';

        var continents_set = 'Z,Zz|AS,Asia|AF,Africa|EU,Europe|OC,Australia/Oceania|CA,Caribbean|SA,South America|NA,North America';
            ...
            if(aux != null) {
                ...
                for(var j=0;j<=temp_aux.length-1;j++){
                    ...
                    if(temp_aux[j] == second[0]) {
                        ...
                        current_selected = ' class="selected" selected="selected"  ';
        ...
        mycellthree.style.padding = "15px 0px";

        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="continent" />';

        mycellfour.align = "center";
        ...
        if (firstElement == true) {
            ...
            document.getElementById("limitation-"+currentid+"logical").style.display = "none";

            // don't display the and/or for the first element

        }

        //, onremove: onRemoveContinent
        ...
        window.setTimeout(function(){
            ...
            //ADAG("#limitation-"+currentid).find('.holder .bit-box').remove();
       ...
       mycell.innerHTML = '<select name="limitation['+currentid+'][logical]" id="'+myrow.id+'logical" style="display:'+display+';"><option value="OR"><?php echo JText::_('ADAG_OR'); ?></option><option value="AND"><?php echo JText::_('ADAG_AND'); ?></option></select>';
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_COUNTRY');?>';
        ...
        var output = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="is"><?php echo JText::_('ADAG_IS_ANY'); ?></option><option value="isnot"><?php echo JText::_('ADAG_NOT_IS_ANY'); ?></option></select><p /><select id="limitation_select_'+currentid+'" name="limitation['+currentid+'][data][]" class="autocomplets">'+outputs+'</select>';
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="country" />';
        ...
        mycell.innerHTML = '<select name="limitation['+currentid+'][logical]" id="'+myrow.id+'logical" style="display:'+display+';"><option value="AND"><?php echo JText::_('ADAG_AND'); ?></option><option value="OR"><?php echo JText::_('ADAG_OR'); ?></option></select>';
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_CREGION');?>';

        //var myString = getCountriesWithRegions();
		...
		saved_country = "";
		saved_city = "";
		if(aux != null){// already saved country / city
			temp_aux = aux.split('|');
			saved_country = temp_aux[0];
			saved_city = temp_aux[1];
        }
		
		for(var i=0; i <= mySplitResult.length-1; i++){
            var second = mySplitResult[i].split(",");
			saved_country = saved_country.replace(second[0], second[1]);
        }
		
		
		var saved_region = new Array();
		var getRegions = "";
		var poz = 0;
        if(temp_aux != null){
			var getRegs = getRegionByCountryCode(temp_aux[0]);
			country_code = temp_aux[0];
			var temp_aux = temp_aux[1].split(",");
			for(var j = 0; j <= getRegs.length-1; j++){
                var splitEm = getRegs[j].split(",");	
                for(var k = 0; k <= temp_aux.length-1; k++){
					if(temp_aux[k].toString() == splitEm[0].toString()){
						saved_region[poz++] = splitEm[1];
                    }
                }
            }
		...
		// for country region -----------------------------------------------------------
		var outputs = "<table><tr><td><div class=\"ui-widget\"><?php echo JText::_('ADAG_COUNTRY');?></td><td><input id=\"country"+currentid+"\" size=\"50\" type=\"text\" value=\""+saved_country+"\" "+readonly_input+" onkeyup=\"javascript:setOnlyOneCountry('country"+currentid+"')\" name=\"limitation["+currentid+"][data][]\" />";
		outputs += "</div></td></tr><tr><td><?php echo JText::_('ADAG_REGS');?></td><td><input id=\"region"+currentid+"\" size=\"50\" type=\"text\" value=\""+saved_region+"\" name=\"limitation["+currentid+"][data][]\" /></td></tr></table>";
        // for country region -----------------------------------------------------------
		
        var output = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="is"><?php echo JText::_('ADAG_IS_ANY'); ?></option><option value="isnot"><?php echo JText::_('ADAG_NOT_IS_ANY'); ?></option></select><p />'+outputs;
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="region" />';
        ...
        window.setTimeout(function(){
            ...
            ADAG("#limitation-"+currentid+" .autocomplets").fcbkcomplete({ filter_selected: true, onselect: onSelectItem, onremove: onRemoveItem, firstselected: false, maxitems: 1 });

            ADAG("#updateRegions-"+currentid).fcbkcomplete();
		...
		$(document).ready(function(){
			startAutocompleteCountry();
			startAutocompleteRegion();
		});
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_CCITY');?>';
        ...
        // for country city -----------------------------------------------------------
		var outputs = "<table><tr><td><div class=\"ui-widget\"><?php echo JText::_('ADAG_COUNTRY');?></td><td><input id=\"country"+currentid+"\" size=\"50\" type=\"text\" value=\""+saved_country+"\" "+readonly_input+" onkeyup=\"javascript:setOnlyOneCountry('country"+currentid+"')\" name=\"limitation["+currentid+"][data][]\" />";
		outputs += "</div></td></tr><tr><td><?php echo JText::_('ADAG_CITY');?></td><td><input id=\"city"+currentid+"\" size=\"50\" type=\"text\" value=\""+saved_city+"\" name=\"limitation["+currentid+"][data][]\" /></td></tr></table>";
        var output = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="=="><?php echo JText::_('ADAG_IS_ANY'); ?></option><option value="!="><?php echo JText::_('ADAG_NOT_IS_ANY'); ?></option></select><p />'+outputs;
		// for country city -----------------------------------------------------------
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="city" />';
        ...
        window.setTimeout(function(){
            ...
            if(aux){
                ...
                ADAG("#limitation-"+currentid).find('.holder:first .maininput').hide();
        ...
        window.setTimeout(function(){
			...
			if(ADAG('#limitation-'+ currentid +'-data1').val() == null) {

				ADAG('#city-container-' + currentid).text('<?php echo JText::_('ADAG_BEFORE_COUNTRY'); ?>');

            } else {

                var the_country = ADAG('#limitation-'+ currentid +'-data1').val();

                if(typeof(temp_aux) != 'undefined') {

                    var s_city = '';

                    for(var i=1;i<=temp_aux.length-1;i++){

                        s_city += '<option class="selected" selected="selected" value="'+temp_aux[i]+'">'+temp_aux[i]+'</option>';

                    }

                } else { s_city = '';}

                ADAG('<select name="limitation['+currentid+'][data][]" id="limitation-'+currentid+'city">' + s_city + '</select>').appendTo('#city-container-' + currentid);

                window.setTimeout(function(){

                    ADAG("#limitation-"+currentid+"city").fcbkcomplete({ json_url: "<?php echo JUri::root().$configs->countryloc."/country-"; ?>" + the_country + ".txt", filter_selected: true, onselect: onSelectItem, onremove: onRemoveItem, firstselected: false});
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_LATLONG');?>';
        ...
        var output = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="==">is within</option><option value="!=">is not within</option></select><p />'+outputs;
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="latitude" />';
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_DMA');?>';
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="dma" />';
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_USAREA');?>';



        //var outputs="<input size=\"40\" onkeyup=\"checkChannel();\" type=\"text\" name=\"limitation["+currentid+"][data][]\" class=\"input20\" />";

        var outputs = "<select class='upd8usarea autocomplets' name=\"limitation["+currentid+"][data][]\" id=\"limitation-"+currentid+"usarea\" class='input20'></select></td></tr></table><input type='hidden' name='limitation["+currentid+"][type]' value='usarea' />";

        var output = '<select name="limitation['+currentid+'][option]" id="'+myrow.id+'option"><option value="=="><?php echo JText::_('ADAG_IS_ANY'); ?></option><option value="!="><?php echo JText::_('ADAG_NOT_IS_ANY'); ?></option></select><p />'+outputs;

        //<option value="=~"><?php echo JText::_('ADAG_CONTAINS'); ?></option><option value="!~"><?php echo JText::_('ADAG_NOT_CONTAINS'); ?></option>
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="usarea" />';
        ...
        mycelltwo.innerHTML = '<?php echo JText::_('ADAG_POSTAL_COD');?>';

        var myString = getCountryCodes();

        var outputs = "<input size=\"40\" type=\"text\" onkeyup=\"checkChannel();\" name=\"limitation["+currentid+"][data][]\" class=\"input20\" value=\"<?php echo JText::_('ADAG_ZIPNOTE');?>\" />";

        //window.setTimeout(function(){ ADAG('#limitation-'+ currentid +' p').after('<?php echo JText::_('ADAG_ZIPNOTE');?>'); },1);
        ...
        //var outputs ="<select class='upd8postalcode autocomplets' name=\"limitation["+currentid+"][data][]\" id=\"limitation-"+currentid+"postalcode\" class='input20'></select>";

        /*if(aux != null) {

            window.setTimeout(function(){

                ADAG('<option class="selected" selected="selected" value='+aux+'>'+aux+'</option>').appendTo('#limitation-'+ currentid +' .upd8postalcode');
        ...
        mycellfour.innerHTML = '<img onclick="javascript:deletelim(\''+myrow.id+'\');" src="components/com_adagency/images/delete-icon.gif" title="remove limitation" alt="remove limitation" /><input type="hidden" name="limitation['+currentid+'][type]" value="postalcode" />';
...
function onSelectItem(param){
	...
	window.setTimeout(function(){
		...
		if(param.hasClass('postalcode')) {//param.hasClass('usarea')||
            ...
            //alert(currentid[1]);alert(val);alert("'updateRegions-"+currentid[1]+"'");

            updateRegions(id.toString(),currentid[1],val);

        }

		else if(param.hasClass('city')){

			if(param.find('.holder:eq(1)').length == 0){

                var the_id = param.attr('id').split('-');

                the_id = the_id[1];

                ADAG('#city-container-' + the_id).text('');

                ADAG('<select name="limitation['+the_id+'][data][]" id="limitation-'+the_id+'city"></select>').appendTo('#city-container-' + the_id);

                window.setTimeout(function(){
                    var the_country = param.find('.holder:first .bit-box').attr('rel');

                    ADAG("#limitation-"+ the_id +"city").fcbkcomplete({ json_url: "<?php echo JUri::root().$configs->countryloc."/country-"; ?>" + the_country + ".txt", filter_selected: true, onselect: onSelectItem, onremove: onRemoveItem, firstselected: false});
...
function onRemoveItem(param){
    ...
    /*if(param.find('.holder .bit-box').length<=1) {
    ...
    if(param.hasClass('region')||param.hasClass('postalcode')) {//||param.hasClass('usarea')
    ...
    if(param.hasClass('city')){

        //param.find('div[id^=city-container] .maininput');

        param.find('.holder').each(function(){

            //alert(ADAG(this).find('.bit-box').length);

            if(ADAG(this).find('.bit-box').length == 1) {

                ADAG(this).find('.maininput').show();
...
function initSanitizeLi(){
    ...
    ADAG('#opts tr').each(function(){
        ...
        if(!ADAG(this).hasClass('city')){

            if((!ADAG(this).hasClass('region'))&&(!ADAG(this).hasClass('city'))){
...
function sanitizeLi(param){
    ...
    //if(param.find('.bit-input').length>1){

        //param.find('.bit-input :not(:last)').remove();
    ...
    //} else {
        ...
        //alert(param.find('.bit-input').length);
...
function removeTyperRegion(){
    ...
    ADAG('#opts tr').each(function(){
        ...
        if(ADAG(this).hasClass('region')){

            ADAG(this).find('.maininput:first').hide();

            //ADAG(this).css('border','2px solid red');
