...
class adagencyAdminModeladagencyAdcode extends JModelLegacy {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function getExistsZone() {
		$db = JFactory::getDBO();
		$sql = "SELECT zoneid FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%affiliate%' LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
	...
	function getExistsZoneWH($w,$h) {
		$db = JFactory::getDBO();
		$sql = "SELECT * FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%affiliate%'";
		$db->setQuery($sql);
	...
	function getExistsAffiliateCampaign() {
		$db = JFactory::getDBO();
		$sql = "SELECT c.id FROM `#__ad_agency_campaign` AS c
				...
				WHERE z.adparams LIKE '%affiliate%'
				LIMIT 1
				";
		$db->setQuery($sql);
		$res = $db->loadResult();
	...
	function getExistsPackage(){
		...
		$sql = "SELECT tid FROM `#__ad_agency_order_type` AS p WHERE p.location IN (
					SELECT `zoneid` FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%affiliate%'
				) LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
		if(intval($res) > 0) { return true; } else { return false; }
	...
	function getCampsByAid($adv_id, $distinct = 0) {
		...
		$sqls = "
			...
			ON z.zoneid = pz.zone_id
			WHERE c.aid = '".intval($adv_id)."' AND z.adparams LIKE '%affiliate%'
			AND c.status != -1
		";
	...
	function getadcodelistAdvertisers () {
		if (empty ($this->_package)) {
			$db = JFactory::getDBO();
