...
class adagencyAdminModeladagencyPopup extends JModelLegacy {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function getExistsZone(){
		$db = JFactory::getDBO();
		$sql = "SELECT zoneid FROM `#__ad_agency_zone` WHERE `adparams` LIKE '%popup%' LIMIT 1";
		$db->setQuery($sql);
		$res = $db->loadResult();
	...
	function createSelectBox($czones, $ad_id, $ad){
        ...
        if(isset($czones)&&(is_array($czones))){
			...
			foreach($czones as $key=>$value){
				...
				$sql = "SELECT z.`zoneid`, z.`z_title`, z.`adparams` FROM `#__ad_agency_zone` z, `#__ad_agency_campaign` c, `#__ad_agency_package_zone` pz WHERE pz.zone_id=z.zoneid and c.otid=pz.package_id and c.id=".intval($key);
				$db->setQuery($sql);
				$db->query();
				$result = $db->loadAssocList();
