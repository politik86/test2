...
defined( '_JEXEC' ) or die( 'Restricted access' );
			...
			if(isset($current->sets)){
				foreach($current->sets as $element) {
					...
					if(is_array($element->data)) {
						...
						//echo "<pre>";var_dump($temp_set);die();
					} else { $temp_set = 'null'; }
	...
	if(isset($configs->geoparams['allowgeoexisting']) && ($configs->geoparams['allowgeoexisting'] == '1') && ((!isset($current->sets))||(count($current->sets) == 0))){
		$output3 = '
			ADAG(function(){
