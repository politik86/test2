...
class adagencyModeladagencyFloating extends JModelLegacy {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function getCampsByAid($adv_id, $distinct = 0) {
		...
		$sqls = "
			...
			WHERE c.aid = ".intval($adv_id)." AND z.adparams LIKE '%floating%'
			AND c.status != -1
		";
	...
	function store () {
		...
		if(!isset($data['parameters']["align"])){$data['parameters["align"]']=NULL;}
		if(!isset($data['parameters']["border"])){$data['parameters["border"]']=NULL;}
		if(!isset($data['parameters']["border_color"])){$data['parameters["border_color"]']='000000;';}
		if(!isset($data['parameters']["bg_color"])){$data['parameters["bg_color"]']='FFFFFF';}
