...
ADAG(function() {
	...
	<?php
		...
		case "TextLink":
			...
			if(isset($txtimageurl)) {$imagetxtcode='<img class="standard_adv_img" style="width: 100%;" src="'. $txtimageurl .'" border="0" alt="'.$banner_row->parameters['img_alt'].'" />'; } else {$imagetxtcode='';}
		...
		case "Jomsocial":
			...
			$advertiser_id = $banner_row->advertiser_id;
			
			$directory 	= "/images/stories/".$imgfolder."/".$advertiser_id;
			$absolutepath = JPATH_SITE;
			$image_folder = "{$absolutepath}{$directory}";
			$image_path = "images/stories/".$imgfolder."/".$advertiser_id."/";
			
			$output  =  '<div class="ada-joms-preview-avatar">';
										
			if(isset($banner_row->image_url) && trim($banner_row->image_url) != ""){
				$output .=  '<img src="'.$image_path.$banner_row->image_url.'" id="preview-image-image-url" alt="ad avatar image" title="ad avatar image">';
			}
							
			$output .= '</div>
						<div class="ada-joms-preview-content">
							<h3 class="uk-h3" style="color: #2196f3" id="preview_ad_headline">
								'.$banner_row->ad_headline.'
							</h3>
							<span id="preview_promote_url" class="ada-joms-preview-link">
								'.$banner_row->target_url.'
							</span>
							<p id="preview_ad_text">
								'.$banner_row->ad_text.'
							</p>
							<div id="preview_content_image">';
								
								
			if(isset($banner_row->image_content) && trim($banner_row->image_content) != ""){
				$output .=  '<img src="'.$image_path.$banner_row->image_content.'" id="preview-image-image-content" alt="ad content image" title="ad content image">';
			}			
								
			$output .= '	</div>
							<div class="uk-float-left">';
			
			if($params["show_sponsored_stream_info"] == "1"){
				$output .= JText::_("ADAG_SPONSORED_STREAM");
			}
			
			$output .= '	</div>
							<div class="uk-float-right">';

			if($params["show_create_ad_link"] == "1"){
				$output .= JText::_("ADAG_CREATE_AN_AD");
			}
