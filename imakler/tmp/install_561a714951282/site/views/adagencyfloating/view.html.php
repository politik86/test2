...
class adagencyViewadagencyFloating extends JViewLegacy {
	...
	function editForm($tpl = null) { 
		global $mainframe;
		$helper = new adagencyModeladagencyFloating();
		...
		if (($ad->advertiser_id==$advertiser_id) && ($ad->media_type!="Floating")) die('This banner id is not a Floating banner');
		}
		//check for valid id of the banner		
		$isNew = ($ad->id < 1);		
		if (!$isNew) {
		...
		if ($adv_id) {
			$camps = $this->getModel("adagencyFloating")->getCampsByAid($adv_id);
		} else $camps='';
		$these_campaigns = $this->getModel("adagencyFloating")->getSelectedCamps($advertiser_id, $ad->id);
	
		if(isset($ad->id)&&($ad->id != 0)) { $channel = $this->getModel("adagencyFloating")->getChannel($ad->id); } else { $channel = NULL; }
		
		/*$czones = $this->getModel("adagencyFloating")->processCampZones($camps);
		$czones = $this->getModel("adagencyFloating")->createSelectBox($czones,$ad->id);*/
		
		$czones = $this->getModel("adagencyFloating")->processCampZones($camps);		
		@$ad->width=$size_selected['0'];
		@$ad->height=$size_selected['1'];
        $czones_select = $this->getModel("adagencyFloating")->createSelectBox($czones, $ad->id, $ad);
		$campaigns_zones = $this->getModel("adagencyFloating")->getCampZones($camps);
		
		$itemid_cpn = $this->getModel("adagencyConfig")->getItemid('adagencycpanel');

        $camps = $this->getModel("adagencyFloating")->getCampsByAid($adv_id, 1);
